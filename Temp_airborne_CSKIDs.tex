...
Note that 
\[\nabla_{(A}{}^{A'}H_{\vert A'\vert BCD)} = 6\Theta B_{ABCD} \]
...
Decomposing, we have 
\begin{multline}
\nabla_{\bm\tau} F_{B'BCD} + 2 \tau^{A}{}_{B'} \tau^{FA'} \mathcal{D}_{AF}F_{A'BCD} = \tfrac{2}{3} \nabla_{\xi}{}\phi_{BCDA} \tau^{A}{}_{B'} - 4 \Lambda  B_{BCDA}\tau^{A}{}_{B'} + 2 \Theta  B_{CD}{}^{FG} \tau^{A}{}_{B'} \phi_{BAFG} \\+ 2 \Theta B_{BD}{}^{FG}  \tau^{A}{}_{B'} \phi_{CAFG} + 2 \Theta B_{BC}{}^{FG}  \tau^{A}{}_{B'} \phi_{DAFG} \\ - 14 \Theta \tau^{A}{}_{B'} B_{(BC}{}^{FG}\phi_{DA)FG}  -  \tfrac{1}{2} \tau^{A}{}_{B'} \phi_{(BCD}{}^{F}Q_{A)F} \\+ \tfrac{2}{3} \tau^{A}{}_{B'} H^{A'}{}_{(B}{}^{FG}\nabla_{|FA'|}\phi_{CDA)G}.
\end{multline}
Hence, if $F_{A'ABC}=H_{A,ABC}=B_{ABCD}=0$ on $\mathcal{S}$, then we will also have $\nabla_{\bm\tau}F_{A'ABC}=0$ on $\mathcal{S}$ provided 
\[ \nabla_{\bm\xi}\phi_{ABCD}=0\]
on $\mathcal{S}$. Decomposing the covariant derivative, using the evolution equation for rescaled Weyl () along with \eqref{InitialCondition} we have 
\[ \nabla_{\xi}\phi_{ABCD} =  \xi \mathcal{D}_{D}{}^{F}\phi_{ABCF} + \tfrac{3}{2} \xi^{FG} \mathcal{D}_{FG}\phi_{ABCD} \]
